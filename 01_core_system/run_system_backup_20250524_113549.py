...
def setup_router(models):
    """
    Configura o roteador de modelos.
    
    Args:
        models: Dicionário de {nome_modelo: instância_modelo}
        
    Returns:
        Instância do roteador de modelos
    """
    _, ModelRouter = import_wrapper_and_router()
    
    if not models:
        logger.error("Nenhum modelo disponível para o roteador")
        return None
    
    # Configura o roteador com os modelos carregados
    capabilities = {}
    for name, model in models.items():
        # Detecta capacidades baseadas no nome do modelo
        model_caps = ["general"]
        
        if any(x in name.lower() for x in ["chat", "instruct", "assistant"]):
            model_caps.append("chat")
        
        if any(x in name.lower() for x in ["code", "coder", "programming"]):
            model_caps.append("code")
            
        if any(x in name.lower() for x in ["math", "qwen"]):
            model_caps.append("math")
            
        capabilities[name] = model_caps
    
    # Prioridades baseadas no tamanho/capacidade
    performance_scores = {
        name: 7 if "7b" in name.lower() else
              8 if "13b" in name.lower() else
              9 if "30b" in name.lower() else
              5 if "phi-3" in name.lower() else
              6 if "phi-2" in name.lower() else
              6 if "falcon" in name.lower() else
              5
    ...
    logger.info(f"Roteador configurado com {len(models)} modelos")
    return router
